...
;Variables
  ...
  !define PRODUCT_NAME "Electrum Crown"
  !define PRODUCT_WEB_SITE "https://github.com/Crowndev/electrum-crown"
  !define PRODUCT_PUBLISHER "crown.tech"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-crown-setup.exe"
  ...
  VIAddVersionKey LegalCopyright "2013-2020 ${PRODUCT_PUBLISHER}"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-crown\icons\electrum-crown.ico"
...
Section
  ...
  File /r "dist\electrum-crown\*.*"
  File "c:\electrum-crown\icons\electrum-crown.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-crown.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-crown-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-crown.ico"
