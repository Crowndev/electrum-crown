...
from . import ecc
from .bitcoin import (hash_160, hmac_oneshot, NetworkConstants, XPRV_HEADERS, XPRV_HEADERS_INV, XPUB_HEADERS,
                      XPUB_HEADERS_INV, BIP32_PRIME, rev_hex, int_to_hex, EncodeBase58Check, DecodeBase58Check)
...
def xprv_header(xtype: str, *, net=None) -> bytes:
    ...
    return XPRV_HEADERS[xtype].to_bytes(length=4, byteorder="big")


def xpub_header(xtype: str, *, net=None) -> bytes:
    if net is None:
        net = NetworkConstants
    return XPUB_HEADERS[xtype].to_bytes(length=4, byteorder="big")
